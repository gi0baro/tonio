...
def test_run_process_check(run):
    def _run():
        return (yield tonio.run_process([sys.executable, '-c', 'import sys; sys.exit(3)'], check=False))
...
def test_run_process_check_raises(run):
    def _run():
        return (yield tonio.run_process([sys.executable, '-c', 'import sys; sys.exit(3)']))
...
def test_kill(run):
    def _run():
        proc = yield tonio.open_process([sys.executable, '-c', 'import time; time.sleep(10)'])
        proc.kill()
