...
from .._tonio import ResourceBroken, TLSStream as _TLSStream, Waiter, WouldBlock, get_runtime
...
class _SSLProxy:
    ...
    def server_side(self) -> bool:
        return self._inner.server_side
    ...
    def read(self, len: int = 1024, buffer: Any = None) -> bytes:
        with self._lock:
            return self._inner.read(len, buffer)
    ...
    def _pending(self) -> bool:
        with self._lock:
            return bool(self._inner.pending() or self._ingress.pending)

    @property
    def _ingress_pending(self) -> int:
        with self._lock:
            return self._ingress.pending

    def _ingress_write(self, data) -> None:
        with self._lock:
            self._ingress.write(data)

    def _ingress_write_eof(self) -> None:
        with self._lock:
            self._ingress.write_eof()

    def _read(self, max_bytes: int) -> tuple[Any, bool, bytes]:
        return self._step(self._inner.read, max_bytes)

    def _read_nowait(self, max_bytes: int) -> tuple[Any, bool]:
        with self._lock:
            try:
                return self._inner.read(max_bytes), False
            except _stdlib_ssl.SSLWantReadError:
                return None, True

    def _write(self, data) -> tuple[Any, bool, bytes]:
        return self._step(self._inner.write, data)

    def _do_handshake(self) -> tuple[Any, bool, bytes]:
        return self._step(self._inner.do_handshake)

    def _unwrap(self) -> tuple[Any, bool, bytes]:
        return self._step(self._inner.unwrap)

    def _step(self, fn, *args) -> tuple[Any, bool, bytes]:
        with self._lock:
            ret, want_read = None, False
            try:
                ret = fn(*args)
            except _stdlib_ssl.SSLWantReadError:
                want_read = True
            return ret, want_read, self._egress.read()


class TLSStreamWatcher:
    __slots__ = ['_lock', '_pending', '_arm', '_handoff']

    def __init__(self, lock: Lock, pending, arm):
        self._lock = lock
    ...
    def __enter__(self):
        self._handoff = self._lock.acquire()
        return self

    def waiter(self, timeout: int | None = None) -> Waiter | None:
        if self._handoff is not None:
            return self._handoff.waiter(timeout)
        if self._pending():
            return None
    ...
    def ready(self) -> bool:
        if self._handoff is not None:
            return self._handoff.is_set() and self._pending()
        return self._pending() or self._arm() is None

    def __exit__(self, *args):
        if self._handoff is None or not self._handoff.set():
            self._lock.release()
...
class TLSStream(_Stream, _TLSStream):
    ...
    def _recv(self) -> Coro[None]:
        recv_count = self._recv_count
        with (yield self._lock_recv()):
            if recv_count == self._recv_count:
                data = yield self.transport.receive_some()
                if not data:
    ...
    def _send(self, data) -> Coro[None]:
        with (yield self._lock_send()):
            try:
                self._egress_stack.extend(data)
                data = bytes(self._egress_stack)
                self._egress_stack.clear()
                yield self.transport.send_all(data)
            except:
                self._set_broken()
                raise

    def _ssl_dance(self, ssl_fn, *args) -> Coro[Any]:
        while True:
            try:
    ...
    def send_all(self, data: bytes | bytearray | memoryview) -> Coro[None]:
        ...
        if not data:
            return
        yield self._ssl_dance(self._ssl._write, data)

    def receive_some(self, max_bytes: int | None = None) -> Coro[bytes | bytearray]:
        self._check_ready()
        if max_bytes is None:
            max_bytes = max(self._recv_est_size, self._ssl._ingress_pending)
        try:
            ret = yield self._ssl_dance(self._ssl._read, max_bytes)
            return ret
