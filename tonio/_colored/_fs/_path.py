...
def _wraps_async(  # type: ignore[explicit-any]
    ...
    def decorator(fn: Callable[P, T]) -> Callable[P, Awaitable[T]]:
        @wraps(fn)
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> Coroutine[T]:
...
) -> Callable[Concatenate[PathT, P], Awaitable[list[PathT]]]:
    @_wraps_async(fn)
...
class Path(pathlib.PurePath):
    ...
    def __repr__(self) -> str:
        return f'tonio.colored.fs.Path({str(self)!r})'
